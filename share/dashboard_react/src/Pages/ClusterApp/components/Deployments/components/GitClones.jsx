import { VStack, HStack, Input, Heading, Text, Flex } from '@chakra-ui/react'
...
export default React.memo(function GitClones({
  ...
  const handleAddItem = () => {
    setFormData(prevState => ([...prevState, { repo: "", branch: "", dest: "", user: "", pass: "" }]));
  };
      ...
      <VStack spacing={3} align="stretch">
        ...
        rows?.map((gc, index) => (
          <HStack key={`row_${index}`}>
            ...
            <TextForm confirmTitle={defaultConfirmText} name={`row_${index}.dest`} placeholder="Destination" value={gc.dest} onSave={(value) => onRowArrayChange(fieldName, index, "dest", value)} />
      ...
      <VStack spacing={3} align="stretch">
        ...
        {formData.map((gc, index) => (
          <HStack key={`new_${index}`}>
            ...
            <Input name={`new_${index}.dest`} placeholder="Destination" value={gc.dest} onChange={(e) => handleArrayChange(index, "dest", e.target.value)} />

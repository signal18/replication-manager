 ...
 *
 * New JSON contract (from Go patch):
 *   Each table in shardSchema now carries:
 *     table_parents   []TableLink  – edges this table → other  (outgoing FK)
 *     table_children  []TableLink  – edges other → this table  (incoming FK)
 *     size_weight_pct float64      – storage % within schema
 *
 *   TableLink shape:
 ...
 *     relation_source "foreign_key" | "column_name_match" | "workload_query"
 *     cardinality     "1-1" | "1-N" | "N-N"
 *     join_weight_pct float64 (optional)
 *
 * Theme: reads colorMode from @chakra-ui/react.
 *   Uses var(--text-color), var(--secondary-gray-color), var(--gray-color),
 *   var(--darkgray-color) from the existing SCSS for toolbar/panel chrome.
 *   Graph-specific palette constants are defined as [lightValue, darkValue]
 *   pairs selected via isDark.
 *
 * No external graph library — pure SVG with a minimal force simulation.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const T = {
  ...
  // Edge colours — same both modes (they sit on the SVG background, not on cards)
  edgeFk: '#3B8ADD',
  edgeNm: '#EF9F27',
  edgeWq: '#1D9E75',
}

// Schema-bucket colours — [fill-L, text-L, border-L, fill-D, text-D, border-D]
const SCHEMA_PAL = [
...
// Sync badge — [bg-L, fg-L, bd-L, bg-D, fg-D, bd-D]
const SYNC = {
...
function syncMeta(value, dark) {
  ...
  const c  = SYNC[k] || SYNC['']
  const off = dark ? 3 : 0
  return { bg: c[0+off], fg: c[1+off], bd: c[2+off],
    icon: k === 'OK' ? '✓' : k === 'ER' ? '✕' : k === 'NA' ? '⊘' : '○',
    label: k === 'OK' ? 'OK' : k === 'ER' ? 'ERROR' : k === 'NA' ? 'N/A' : '—',
    tip: k === 'OK' ? 'In sync on all replicas'
       : k === 'ER' ? 'Checksum mismatch'
       : k === 'NA' ? 'No unique key or process error'
       : 'Not yet checksummed',
  }
}
...
// Build a flat de-duplicated edge list from table_parents on every table.
// Each edge: { childTable, childSchema, parentTable, parentSchema,
//              source, cardinality, joinWeight, childCols, parentCols, name }
...
function runLayout(tables, edges, mode) {
  ...
  for (let tick = 0; tick < FORCE_TICKS; tick++) {
    ...
    tables.forEach(t => {
      ...
      const nh = nodeH(t,mode)/2, hw = nw/2
...
function mkEdgePath(ax, ay, aw, ah, bx, by, bw, bh) {
  const c1x=ax+aw/2, c1y=ay+ah/2, c2x=bx+bw/2, c2y=by+bh/2
  const [x1,y1]=rectPt(c1x,c1y,aw,ah,c2x,c2y)
  const [x2,y2]=rectPt(c2x,c2y,bw,bh,c1x,c1y)
  const mx=(x1+x2)/2-(c2y-c1y)*0.06, my=(y1+y2)/2+(c2x-c1x)*0.06
  return { d:`M${x1.toFixed(1)},${y1.toFixed(1)} Q${mx.toFixed(1)},${my.toFixed(1)} ${x2.toFixed(1)},${y2.toFixed(1)}`, mx, my }
}

// ─── Tiny DB cylinder SVG icon ────────────────────────────────────────────────
...
function SectionLabel({ label, dark }) {
  return (
    <div style={{
      ...
      textTransform:'uppercase', color: p(T.textMut,dark),
    }}>{label}</div>
  )
}
...
export default function SchemaGraph({ tables = [], onChecksum, onRepair }) {
  ...
  // Build flat edge list once from table_parents
  ...
  const filtered = useMemo(() => {
    let rows = tables
    if (syncFilter !== 'all')
      rows = rows.filter(t => (t.table_sync||'').toUpperCase() === syncFilter)
    if (search.trim()) {
      const s = search.trim().toLowerCase()
      rows = rows.filter(t =>
        t.table_name?.toLowerCase().includes(s) ||
        t.table_schema?.toLowerCase().includes(s)
      )
    }
    return rows
  }, [tables, syncFilter, search])

  const visNames    = useMemo(() => new Set(filtered.map(t => t.table_name)), [filtered])
  const filtEdges   = useMemo(() =>
  ...
  const syncCounts = useMemo(() => {
    ...
    tables.forEach(t => { const k=(t.table_sync||'').toUpperCase(); c[k]=(c[k]||0)+1 })
    return c
  }, [tables])

  const totalBytes = useMemo(() => tables.reduce((s,t)=>s+(t.data_length||0)+(t.index_length||0),0),[tables])
  ...
  const FILTERS = useMemo(() => [
    ...
    ...['OK','ER','NA',''].map(k => {
      const m = syncMeta(k, dark)
      const display = k===''?'Unchecked':m.label
      const count = syncCounts[k]||0
      return { key:k||'__none__', filterKey:k, bg:m.bg, fg:m.fg, bd:m.bd,
        label:`${display} (${count})` }
    }),
  ], [syncCounts, dark])
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
        {[
          ['Tables',     `${filtered.length} / ${tables.length}`],
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          <TBtnGroup dark={dark}>
            <TBtn active={view==='graph'}      onClick={()=>setView('graph')}      dark={dark}>⬡ Graph</TBtn>
            <TBtn active={view==='attributes'} onClick={()=>setView('attributes')} dark={dark}>☰ Attributes</TBtn>
          </TBtnGroup>
          ...
          <input
            ...
            style={{
              padding:'5px 10px', minWidth:150,
              border:`1px solid ${p(T.ctrlBorder,dark)}`,
              borderRadius:6, fontSize:12, outline:'none',
              background: p(T.ctrlBg,dark),
              color: p(T.textPri,dark),
            }}
          />
        </div>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
          <span style={{ fontSize:11, color:'var(--darkgray-color)', fontWeight:600 }}>Sync:</span>
          {FILTERS.map(f => {
            const isActive = syncFilter === (f.filterKey ?? f.key)
            return (
              <button key={f.key}
                onClick={() => setSyncFilter(f.filterKey ?? f.key)}
                style={{
                  padding:'3px 10px', borderRadius:20, fontSize:11, cursor:'pointer',
                  fontWeight: isActive ? 700 : 400,
                  border:`1px solid ${isActive&&f.bd ? f.bd : p(T.ctrlBorder,dark)}`,
                  background: isActive&&f.bg ? f.bg : 'transparent',
                  color: isActive&&f.fg ? f.fg : p(T.textSec,dark),
                  transition:'all .12s',
                }}
              >{f.label}</button>
            )
        ...
        {/* Row 3: edge legend (graph only) */}
        {view==='graph' && (
          <div style={{ display:'flex', gap:14, flexWrap:'wrap', alignItems:'center' }}>
            {[
              { col:T.edgeFk, dash:null,  label:'Foreign key' },
              { col:T.edgeNm, dash:'7 4', label:'Name match' },
              { col:T.edgeWq, dash:'3 6', label:'Workload join' },
            ].map(it => (
              <div key={it.label} style={{ display:'flex', alignItems:'center', gap:6 }}>
                <svg width="28" height="10" style={{ overflow:'visible' }}>
                  <line x1="0" y1="5" x2="28" y2="5" stroke={it.col} strokeWidth="1.5"
                    strokeDasharray={it.dash||undefined}/>
                  <polygon points="24,2.5 28,5 24,7.5" fill={it.col}/>
                </svg>
                <span style={{ fontSize:11, color:p(T.textSec,dark) }}>{it.label}</span>
              </div>
            ))}
            <div style={{ width:1, height:14, background:p(T.ctrlBorder,dark) }}/>
            {[{label:'1:1',c:'#1D9E75'},{label:'1:N',c:'#3B8ADD'},{label:'N:N',c:'#D85A30'}].map(b=>(
      ...
      {view === 'graph'
        ? <GraphCanvas
            tables={filtered} edges={filtEdges}
            graphMode={graphMode} weightMode={weightMode}
            schemas={schemas} dark={dark}
            onChecksum={onChecksum} onRepair={onRepair}
          />
        : <AttributeList
            tables={filtered} schemas={schemas} dark={dark}
            onChecksum={onChecksum} onRepair={onRepair}
          />
      }
    </div>
      ...
      <div style={{ position:'absolute', top:10, right:10, zIndex:20, display:'flex', flexDirection:'column', gap:5 }}>
        {[
          ...
          <button key={b.l} onClick={b.f} style={{
            width:28, height:28, border:`1px solid ${p(T.ctrlBorder,dark)}`,
      ...
      >
        <defs>
          {[['fk',T.edgeFk],['nm',T.edgeNm],['wq',T.edgeWq]].map(([id,c])=>(
            <marker key={id} id={`arr-${id}`}
              ...
              <path d="M2 2L8 5L2 8" fill="none" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </marker>
          ))}
        </defs>
        ...
        <g transform={`translate(${pan.x*zoom},${pan.y*zoom}) scale(${zoom})`}>
          ...
          {edges.map((e, i) => {
            ...
            const {d,mx,my}=mkEdgePath(ap.x-nw/2,ap.y-ah/2,nw,ah,bp.x-nw/2,bp.y-bh/2,nw,bh)
            ...
            return (
              ...
              >
                ...
                <path d={d} fill="none" stroke={col} strokeWidth={sw}
                  strokeDasharray={edgeDash(e.source)||undefined} opacity={op}
                  markerEnd={`url(#arr-${edgeMarkerId(e.source)})`}
          ...
          {tables.map(t => {
            ...
            const nx=pp.x-nw/2, ny=pp.y-nh/2
            ...
            const pkSet=new Set(
              (t.table_indexes||[]).filter(ix=>ix.name==='PRIMARY')
            ...
            return (
              ...
              >
                ...
                {graphMode==='simple' ? <>
                  ...
                  {/* Name */}
                  <text x={nx+22} y={ny+31} fontSize={12} fontWeight={600}
                  ...
                  <DbIcon x={nx+4} y={ny+6} color={pal.text} size={13}/>
                  ...
                  {cols.map((col,ci)=>{
                    ...
                    return (
                      <g key={col.name}>
                        {ci%2===1 && <rect x={nx} y={ry} width={nw} height={ROW_H} fill={p(T.nodeStripe,dark)}/>}
                        {isPk && <>
                          <rect x={nx+5} y={ry+5} width={16} height={13} rx={2}
                            fill={dark?'#2e2214':'#FAEEDA'}/>
                          <text x={nx+13} y={ry+ROW_H/2+4} textAnchor="middle"
                            fontSize={7} fontWeight={800}
                            fill={dark?'#d4914a':'#B07500'} fontFamily="system-ui">PK</text>
                        </>}
                        <text x={nx+(isPk?26:9)} y={ry+ROW_H/2+4}
                        ...
                        <text x={nx+nw-tw/2-7} y={ry+ROW_H/2+4} textAnchor="middle"
                          fontSize={9} fill={tb.fg} fontFamily="monospace">
...
function SelectionPanel({ table: t, pal, edges, dark, onChecksum, onRepair, onDismiss }) {
  // Read both directions from the flat edge list
  ...
  return (
    <div style={{
      ...
      <div style={{ background:pal.fill, padding:'8px 11px', borderBottom:`1px solid ${pal.border}` }}>
        ...
        <div style={{ fontSize:14, fontWeight:700, color:pal.text, display:'flex', alignItems:'center', gap:6, marginTop:1 }}>
      ...
      {/* Relations — from flat edge list, split by direction */}
        ...
        <div style={{ padding:'5px 11px 5px', borderTop:`1px solid ${bd}`, marginTop:2 }}>
          ...
          {parents.slice(0,4).map((e,i)=>(
            <div key={`p${i}`} style={{ display:'flex', gap:4, padding:'1.5px 0', alignItems:'center' }}>
              ...
              <span style={{ flex:1, color:p(T.textSec,dark), fontSize:11,
      ...
      <div style={{ display:'flex', gap:6, padding:'7px 9px', borderTop:`1px solid ${bd}` }}>
        <ActionBtn label="Checksum"
          onClick={()=>onChecksum?.(t.table_schema,t.table_name)}
          bgL="#E6F1FB" fgL="#0C447C" bgD="#0d2340" fgD="#7ab8ef" dark={dark}/>
        <ActionBtn label="Repair"
          onClick={()=>onRepair?.(t.table_schema,t.table_name)}
      ...
      <button onClick={onDismiss} style={{
        ...
        fontSize:11, color:p(T.textMut,dark),
      }}>Dismiss</button>
    </div>
  )
}

// ─── Attribute list (expanded card view) ─────────────────────────────────────
function AttributeList({ tables, schemas, dark, onChecksum, onRepair }) {
  ...
  if (!tables.length) return (
    <div style={{ padding:40, textAlign:'center', color:p(T.textMut,dark) }}>
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))', gap:10 }}>
      {tables.map(t => {
        ...
        // Read links directly from the new JSON fields on Table
        ...
        return (
          <div key={t.table_name} style={{
            ...
            {/* Header */}
            <div onClick={()=>toggle(t.table_name)}
              style={{ background:pal.fill, padding:'10px 13px', cursor:'pointer', userSelect:'none' }}>
                ...
                {[
                  ...
                  <div key={label} style={{ flex:1 }}>
                    ...
                    <div style={{ height:3, background:`${pal.text}22`, borderRadius:2 }}>
                      <div style={{ height:3, width:`${Math.min(val,100)}%`,
                        background:pal.text, opacity:0.5, borderRadius:2 }}/>
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    <div key={col.name} style={{
                      ...
                      {isPk && (
                        <span style={{ fontSize:8, fontWeight:800, padding:'1px 4px', borderRadius:3,
                          ...
                          color:dark?'#d4914a':'#B07500' }}>PK</span>
                      )}
                      ...
                      {!col.nullable && (
                        <span style={{ fontSize:9, color:p(T.textMut,dark),
                          border:`0.5px solid ${p(T.ctrlBorder,dark)}`,
                          padding:'0 3px', borderRadius:2 }}>NN</span>
                      )}
                    </div>
                  )
                })}
                    ...
                    <div key={ix.name} style={{ display:'flex', gap:7, alignItems:'center', padding:'3px 13px' }}>
                      {ix.unique && (
                        <span style={{ fontSize:8, fontWeight:800, padding:'1px 4px', borderRadius:3,
                          ...
                          color:dark?'#7ab8ef':'#0C447C' }}>UQ</span>
                      )}
                ...
                {(parents.length>0||children.length>0) && <>
                  ...
                  {parents.map((lk,i)=>(
                    <div key={`p${i}`} style={{ display:'flex', gap:6, alignItems:'center', padding:'3px 13px' }}>
                      ...
                      {lk.local_columns?.length>0 && (
                        <span style={{ fontSize:9, color:p(T.textMut,dark), fontFamily:'monospace' }}>
                          ({lk.local_columns.join(', ').slice(0,18)}{lk.local_columns.join(', ').length>18?'…':''})
                        </span>
                      )}
                    </div>
                  ))}
                  {children.map((lk,i)=>(
                    <div key={`c${i}`} style={{ display:'flex', gap:6, alignItems:'center', padding:'3px 13px' }}>
                      ...
                      <span style={{ flex:1, color:p(T.textSec,dark) }}>{lk.linked_table}</span>
                      <span style={{ fontSize:9, color:p(T.textMut,dark) }}>{cardLabel(lk.cardinality)}</span>
                      {lk.remote_columns?.length>0 && (
                        <span style={{ fontSize:9, color:p(T.textMut,dark), fontFamily:'monospace' }}>
                          ({lk.remote_columns.join(', ').slice(0,18)}{lk.remote_columns.join(', ').length>18?'…':''})
                        </span>
                      )}
                    </div>
                  ))}

...
Start create an account in https://gitlab.signal18.io
  ...
  const registrationData = [
    ...
    ...(isConnected ? [{
      ...
      help: h(`**Peer Health Mode**\n\nHow peer cluster health status is determined:\n\n- **peering** (default): Direct HTTP polling between instances — gives real-time reachability but scales O(N²)\n- **pulling**: Health comes from peer.json via the back office — lower network overhead, scales O(N), but with a few minutes delay\n\nConfig: \`cloud18-peer-health-mode\``, 'Peer Health Mode'),
      value: (
        <Dropdown
          options={[
            { value: 'peering', label: 'Peering (direct HTTP)' },

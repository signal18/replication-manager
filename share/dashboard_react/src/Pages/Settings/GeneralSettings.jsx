...
function GeneralSettings({ selectedCluster, user, openConfirmModal, onTabChange, monitor }) {
  ...
  const hAdvancedAppVolumeSize = `**Allow Advanced App Volume Size**\n\nWhen enabled, the app canonical storage editor can switch volume sizing from credit-step mode to advanced mode.\n\n- Default mode uses the credit storage step size for easier quota-aligned allocation.\n- Advanced mode allows arbitrary volume sizes while still enforcing the total credit storage budget.\n\nConfig: \`prov-app-volume-allow-advanced-size\``
  ...
  const dataObject = [
    ...
    { key: 'Allow Advanced App Volume Size', help: h(hAdvancedAppVolumeSize, 'Allow Advanced App Volume Size'), value: (<RMSwitch isChecked={selectedCluster?.config?.provAppVolumeAllowAdvancedSize} isDisabled={user?.grants['cluster-settings'] == false} confirmTitle={'Confirm switch settings for prov-app-volume-allow-advanced-size?'} onChange={() => dispatch(switchSetting({ clusterName: selectedCluster?.name, setting: 'prov-app-volume-allow-advanced-size' }))} />) },

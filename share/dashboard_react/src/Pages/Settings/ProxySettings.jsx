...
function ProxySettings({ selectedCluster, user, openConfirmModal }) {
  ...
  const hHaproxyDynamicBackends = `**HAProxy Dynamic Backend Self-Heal**\n\nOn HAProxy 3.4+, automatically recreates missing backends and servers, and reconciles the write-leader/read-server rows, via the HAProxy runtime API — no config reload needed. Requires \`haproxy-mode=runtimeapi\`.\n\nConfig: \`haproxy-runtime-dynamic-backends\``
  ...
  const dataObject = [
    ...
    { key: 'HAProxy Dynamic Backend Self-Heal', help: h(hHaproxyDynamicBackends, 'HAProxy Dynamic Backend Self-Heal'), value: sw('haproxy-runtime-dynamic-backends', 'haproxyRuntimeDynamicBackends') },

...
function Graphite({ chartRef, context, title, title2, target, target2, className, maxExtent = 1000 }) {
  useEffect(() => {
    ...
    const graphite = context.graphite('http://repman.marie-dev.svc.cloud18:10001/graphite')
    const data = graphite.metric(target).alias(title)
    let data2 = null
    if (title2 && target2) {
      data2 = graphite.metric(target2).alias(title2)
    }
    d3.select(chartRef.current).call(function (div) {
      div
        .selectAll('.horizon')
    ...
    context.on('focus', function (i) {
      console.log('context sizeLL', context.size(), i)

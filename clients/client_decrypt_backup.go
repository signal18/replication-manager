...
func runOpenSSLStreamWithPassphraseFD(inputPath, outputPath, password string, args ...string) error {
	input, err := os.Open(inputPath)
	if err != nil {
		return fmt.Errorf("failed to open input file: %w", err)
	}
	...
	if _, err := io.WriteString(pw, password); err != nil {
		_ = pw.Close()
		return fmt.Errorf("failed to write passphrase to pipe: %w", err)
	}
	if err := pw.Close(); err != nil {
		return fmt.Errorf("failed to close passphrase pipe: %w", err)
	}

...
func (repman *ReplicationManager) apiClusterProtectedHandler(router *mux.Router) {
	...
	router.Handle("/api/clusters/{clusterName}/settings/mysql-defaults-cnf", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxGetMySQLDefaultsCnf)),
	))
	router.Handle("/api/clusters/{clusterName}/settings/actions/save-mysql-defaults-cnf", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxSaveMySQLDefaultsCnf)),
	))
...
// handlerMuxGetMySQLDefaultsCnf retrieves the content of the MySQL defaults CNF file
...
// @Router /api/clusters/{clusterName}/settings/mysql-defaults-cnf [get]
func (repman *ReplicationManager) handlerMuxGetMySQLDefaultsCnf(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Access-Control-Allow-Origin", "*")
	...
	// Read the MySQL defaults CNF content using cluster method
	content, err := mycluster.GetMySQLDefaultsCnfContent()
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			http.Error(w, err.Error(), http.StatusNotFound)
			return
		}
		http.Error(w, fmt.Sprintf("Error reading MySQL defaults file: %v", err), http.StatusInternalServerError)
		return
	}
...
// MySQLDefaultsCnfRequest represents the request body for saving MySQL defaults CNF
type MySQLDefaultsCnfRequest struct {
	Content string `json:"content"`
}

// handlerMuxSaveMySQLDefaultsCnf saves the updated content to the MySQL defaults CNF file
...
// @Param Authorization header string true "Insert your access token" default(Bearer <Add access token here>)
// @Param clusterName path string true "Cluster Name"
// @Param body body MySQLDefaultsCnfRequest true "CNF file content"
// @Success 200 {string} string "Successfully saved MySQL defaults CNF"
// @Failure 403 {string} string "No valid ACL"
// @Failure 400 {string} string "Invalid request body"
// @Failure 500 {string} string "Error saving file"
// @Router /api/clusters/{clusterName}/settings/actions/save-mysql-defaults-cnf [post]
func (repman *ReplicationManager) handlerMuxSaveMySQLDefaultsCnf(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Access-Control-Allow-Origin", "*")
	...
	var req MySQLDefaultsCnfRequest
	...
	err = mycluster.WriteMySQLDefaultsCnfContent(req.Content)
	if err != nil {
		http.Error(w, fmt.Sprintf("Error saving MySQL defaults file: %v", err), http.StatusInternalServerError)
		return
	}

	w.WriteHeader(http.StatusOK)
	w.Write([]byte(fmt.Sprintf("Successfully saved MySQL defaults CNF to %s", mycluster.GetMySQLDefaultsPath())))
}

...
func (repman *ReplicationManager) apiClusterProtectedHandler(router *mux.Router) {
	...
	router.Handle("/api/clusters/{clusterName}/actions/query", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxClusterQuery)),
	))
	...
	// SQL Script Job Management
	router.Handle("/api/clusters/{clusterName}/actions/execute-sql-script", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxExecuteSQLScript)),
	))
	router.Handle("/api/clusters/{clusterName}/actions/trigger-scheduled-sql-scripts", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxTriggerScheduledSQLScripts)),
	))
	router.Handle("/api/clusters/{clusterName}/sql-jobs", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxLoadSQLScriptJobs)),
	))
	router.Handle("/api/clusters/{clusterName}/sql-jobs/save", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxSaveSQLScriptJob)),
	))
	router.Handle("/api/clusters/{clusterName}/sql-jobs/{jobName}", negroni.New(
		negroni.HandlerFunc(repman.validateTokenMiddleware),
		negroni.Wrap(http.HandlerFunc(repman.handlerMuxDeleteSQLScriptJob)),
	))
...
// handlerMuxClusterQuery handles SQL query execution at cluster level via proxy or master.
// @Summary Execute SQL query at cluster level
// @Description This endpoint executes an SQL query on the cluster via proxy (if available) or directly on master
// @Tags ClusterQuery
// @Accept json
// @Produce json
// @Param Authorization header string true "Insert your access token" default(Bearer <Add access token here>)
// @Param clusterName path string true "Cluster Name"
// @Param query body object true "Query object" example({"query": "SELECT 1"})
// @Success 200 {object} map[string]interface{} "Query results"
// @Failure 400 {string} string "Bad request"
// @Failure 403 {string} string "No valid ACL"
// @Failure 500 {string} string "Query execution error"
// @Router /api/clusters/{clusterName}/actions/query [post]
func (repman *ReplicationManager) handlerMuxClusterQuery(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Access-Control-Allow-Origin", "*")
	w.Header().Set("Content-Type", "application/json")
	vars := mux.Vars(r)
	mycluster := repman.getClusterByName(vars["clusterName"])
	...
	// Try to execute via proxy first, fallback to master
	var results []map[string]interface{}
	var executeErr error
	var executedVia string

	// Try proxy if available
	if len(mycluster.Proxies) > 0 {
		conn, err := mycluster.GetClusterProxyConn()
		if err == nil && conn != nil {
			defer conn.Close()

			rows, err := conn.Queryx(req.Query)
			if err != nil {
				executeErr = err
			} else {
				defer rows.Close()
	...
	// Fallback to master if proxy failed or not available
	if executedVia == "" {
		master := mycluster.GetMaster()
		if master == nil {
			http.Error(w, "No master available", http.StatusInternalServerError)
			return
		}

		if master.Conn == nil {
			http.Error(w, "Master connection not available", http.StatusInternalServerError)
			return
		}

		rows, err := master.Conn.Queryx(req.Query)
		if err != nil {
		...
		if err := rows.Err(); err != nil {
			http.Error(w, "Error iterating results: "+err.Error(), http.StatusInternalServerError)
			return
		}

...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvDBCapSafetyPct, "prov-db-cap-safety-pct", 15, "Saturation safety margin (per cluster): IsNeedResourceCapUp is raised when ANY DB node consumes at least (1 - pct/100) of its per-node config on ANY axis (cpu/mem/io/disk) -- the worst node, worst axis. Consumption-driven and independent of the dynamic resource change (works on-prem, no plan needed). Default 15 (fire at 85% saturation).")

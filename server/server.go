...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.StringVar(&conf.ProvDBConfigPreserveVars, "prov-db-config-preserve-vars", "aria_log_dir_path,basedir,character_sets_dir,datadir,general_log_file,innodb_data_home_dir,innodb_log_group_home_dir,innodb_undo_directory,log_bin,log_bin_index,log_error,log_slow_query_file,plugin_dir,relay_log,relay_log_index", "Comma-separated list of configuration keys to preserve during config change")
	...
	flags.BoolVar(&conf.ProvDBStartFetchConfig, "prov-db-start-fetch-config", false, "Fetch config every db start. Default FALSE to prevent overload")
	flags.BoolVar(&conf.ProvProxyStartFetchConfig, "prov-proxy-start-fetch-config", false, "Fetch config every proxy start. Default FALSE to prevent overload")

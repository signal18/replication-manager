...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvServicePlanApu, "prov-service-plan-apu", 4, "Service plan in Application Units (APU reservation contract; 1 APU = 1 core / 1GB / 10GB, no IOPS). Default 4 = 2 proxies + phpMyAdmin + 1 client app.")

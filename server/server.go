...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvDBCapBurstDbu, "prov-db-cap-burst-dbu", 1, "TECHNICAL cgroup headroom: burst DBU added on top of the DBU tier when computing the DB container memory cap, so mariadbd's real footprint has room above prov-db-memory and is not OOM-killed. This is NOT overcommit (over-consumption) and NOT the plan; the plan (prov-service-plan-dbu) is never modified. Default 1.")
	...
	flags.IntVar(&conf.ProvDBCapSafetyPct, "prov-db-cap-safety-pct", 15, "Shared safety margin (per cluster) for the two consumed-vs-reference checks, per axis, worst node: CheckResourceConsumedOverConfig (consumed vs the per-node CONFIG -> sets ResourceConsumedOverConfigAxes, whose consequence is to raise the resources) and CheckResourceConsumedOverPlan (consumed vs the per-node PLAN/cap -> sets IsNeedResourceCapUp, whose consequence is to raise the plan). A node counts when it consumes at least (1 - pct/100) of the reference on any axis (cpu/mem/io/disk). Default 15 (fire at 85%).")
	...
	flags.IntVar(&conf.ProvServicePlanDbu, "prov-service-plan-dbu", 0, "Service plan in Database Units (DBU reservation contract; 1 DBU = 1 core / 4GB / 40GB / 1000 IOPS). 0 = AUTO: repman computes it = per-node DBU × node count. Set > 0 to pin the contract explicitly.")

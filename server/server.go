...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvServicePlanDbu, "prov-service-plan-dbu", 0, "DEPRECATED (now a derived rollup): the cluster DBU contract is the sum of per-node prov-db-dbu (PlanByCluster). Kept for back-compat; use prov-db-dbu.")

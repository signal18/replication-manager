...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.SSTWaitMaxLoop, "sst-wait-max-loop", 10, "Maximum number of retry loops for SST job waiting")
	...
	flags.StringVar(&conf.SchedulerJobsExecRemote, "scheduler-jobs-exec-remote", "", "Comma-separated list of tasks to force remote execution via dbjobs (e.g. mysqldump,optimize). Tasks not listed use their default mode.")
	...
	flags.BoolVar(&conf.BackupResticPurgePrune, "backup-restic-purge-prune", true, "Run prune after forget.")
	flags.BoolVar(&conf.BackupResticPurgePruneCompact, "backup-restic-purge-prune-compact", false, "Restic prune --compact (when supported).")
	flags.StringVar(&conf.BackupResticPurgePruneMaxUnused, "backup-restic-purge-prune-max-unused", "", "Restic prune --max-unused size (e.g. 1G).")
	flags.StringVar(&conf.BackupResticPurgePruneMaxRepackSize, "backup-restic-purge-prune-max-repack-size", "", "Restic prune --max-repack-size (e.g. 1G).")
	flags.BoolVar(&conf.BackupResticPurgePruneRepackCacheableOnly, "backup-restic-purge-prune-repack-cacheable-only", false, "Restic prune --repack-cacheable-only.")
	...
	flags.StringVar(&conf.BackupResticAwsEndpoint, "backup-restic-aws-endpoint", "", "Restic backup AWS endpoint (empty = AWS SDK default)")
	flags.StringVar(&conf.BackupResticAwsBucket, "backup-restic-aws-bucket", "", "Restic backup AWS bucket (empty uses repository URL)")
	flags.StringVar(&conf.BackupResticAwsPrefix, "backup-restic-aws-prefix", "", "Restic backup AWS prefix inside the bucket")
	flags.BoolVar(&conf.BackupResticRepoAppendCluster, "backup-restic-repo-append-cluster", true, "Append cluster name to restic repository path")
	...
	flags.StringVar(&conf.BackupResticMountPath, "backup-restic-mount-path", "", "Restic mount path filter (comma-separated absolute paths).")
	flags.StringVar(&conf.BackupResticMountPathTemplate, "backup-restic-mount-path-template", "", "Restic mount path templates (comma-separated).")
	flags.StringVar(&conf.BackupResticMountTimeTemplate, "backup-restic-mount-time-template", "", "Restic mount time template (Go time layout).")
	flags.BoolVar(&conf.BackupResticMountAllowOther, "backup-restic-mount-allow-other", true, "Allow other users to access restic mount.")
	flags.BoolVar(&conf.BackupResticMountNoDefaultPermissions, "backup-restic-mount-no-default-permissions", false, "Ignore default permission handling for restic mount.")
	flags.BoolVar(&conf.BackupResticMountOwnerRoot, "backup-restic-mount-owner-root", false, "Force root ownership for restic mount.")
	flags.BoolVar(&conf.BackupResticMountNoLock, "backup-restic-mount-no-lock", true, "Disable restic repository locking during mount.")
	flags.IntVar(&conf.BackupResticMountVerbose, "backup-restic-mount-verbose", 0, "Restic mount verbosity level (0-3).")
	flags.BoolVar(&conf.BackupResticMountQuiet, "backup-restic-mount-quiet", false, "Enable restic mount quiet mode.")
	flags.BoolVar(&conf.BackupResticMountRecoveryEnabled, "backup-restic-mount-recovery-enabled", true, "Enable restic mount recovery on startup (cleanup stale mounts).")
	...
	flags.StringVar(&conf.BackupMyDumperStreamFile, "backup-mydumper-stream-file", "", "Base filename for mydumper stream output")
	...
	flags.StringVar(&conf.Cloud18MarketplacePricingMode, "cloud18-marketplace-pricing-mode", config.ConstMarketplacePricingModeCsvServicePlan, "Marketplace pricing model. csv-service-plan (DEFAULT) uses a per-cluster service plan downloaded as CSV. global-unit-pricing prices clusters globally from cloud18-marketplace-dbu-price and cloud18-marketplace-app-unit-price (both in Eur), with no per-cluster plan.")
	...
	flags.Float64Var(&conf.Cloud18MarketplaceAppUnitPrice, "cloud18-marketplace-app-unit-price", 0, "Price per Application Unit in Eur, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceMonthlyInfraCost, "cloud18-marketplace-monthly-infra-cost", 0, "Global monthly infrastructure cost, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceMonthlyLicenseCost, "cloud18-marketplace-monthly-license-cost", 0, "Global monthly license cost, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceMonthlySysopsCost, "cloud18-marketplace-monthly-sysops-cost", 0, "Global monthly sysops cost, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceMonthlyDbopsCost, "cloud18-marketplace-monthly-dbops-cost", 0, "Global monthly dbops cost, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	...
	flags.Float64Var(&conf.Cloud18MarketplaceSlaResponseTime, "cloud18-marketplace-sla-response-time", 0, "Global SLA response time, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceSlaRepairTime, "cloud18-marketplace-sla-repair-time", 0, "Global SLA repair time, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplaceSlaProvisionTime, "cloud18-marketplace-sla-provision-time", 0, "Global SLA provision time, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	flags.Float64Var(&conf.Cloud18MarketplacePromotionPct, "cloud18-marketplace-promotion-pct", 0, "Global marketplace promotion percentage, used when cloud18-marketplace-pricing-mode is global-unit-pricing")
	...
	if WithProvisioning == "ON" {
		...
		flags.StringVar(&conf.ProvSphinxMem, "prov-sphinx-memory", "256M", "Sphinx container memory, value with unit e.g. 256M, 1G")
		flags.StringVar(&conf.ProvSphinxDisk, "prov-sphinx-disk-size", "20G", "Sphinx container disk size, value with unit e.g. 20G, 100G")
...
func envViperForScope(scope string) *viper.Viper {
	envViper := viper.New()
	envViper.SetEnvKeyReplacer(strings.NewReplacer("-", "_", ".", "_"))
	envViper.SetEnvPrefix(config.EnvScopePrefix(scope))
	envViper.AutomaticEnv()
	return envViper
}

func (repman *ReplicationManager) applyViperOverrides(dest *config.Config, source *viper.Viper, keys []string) error {
...
func (repman *ReplicationManager) InitConfig(conf config.Config, init_git bool) {
	...
	firstRead := viper.GetViper()
	firstRead.SetConfigType("toml")
	firstRead.SetEnvKeyReplacer(strings.NewReplacer("-", "_", ".", "_"))
	firstRead.SetEnvPrefix(config.EnvScopePrefix("DEFAULT"))
	firstRead.AutomaticEnv()
	...
	_, terminalImmu := ImmuableMap["terminal-session-enabled"]
	if conf.ProvOrchestrator != "opensvc" {
		if !terminalImmu {
			conf.TerminalSessionEnabled = false
		}
	}
	...
	for _, clusterName := range repman.ImmutableClusterList {
		...
		if clRead != nil {
			...
			if clOrch != "opensvc" && conf.TerminalSessionEnabled {
				if v := clRead.Get("terminal-session-enabled"); v == nil {
					conf.TerminalSessionEnabled = false
				}
			}
		}
	...
	tmp_read := firstRead.Sub("default")
	if tmp_read != nil {
		...
		for _, f := range files {
			if f.IsDir() && f.Name() != "graphite" && f.Name() != ".pull" && f.Name() != ".git" && f.Name() != ".config" && f.Name() != ".tmp" {
				...
				if _, err := os.Stat(conf.WorkingDir + "/" + f.Name() + "/" + f.Name() + ".toml"); os.IsNotExist(err) || f.Name() == "overwrite" {
					...
					firstRead.SetConfigFile(conf.WorkingDir + "/" + f.Name() + "/" + f.Name() + ".toml")
					err := firstRead.MergeInConfig()
					if err != nil {
	...
	*repman.Conf = conf
	...
	for _, cl := range repman.ClusterList {
		...
		confs[cl] = repman.GetClusterConfig(firstRead, ImmuableMap, DynamicMap, cl, conf)
		cfgGroupIndex++
	}

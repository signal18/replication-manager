...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvDbBku, "prov-db-bku", 6, "Per-cluster BKU reservation, the backup unit plan (1 BKU = 20 GB of backup disk, nothing else; accounted per cluster, never per node). Backup storage above the plan is over-commit: billed, never blocked. Local BKU = the cluster's backup cache and archive; remote BKU = what is archived on S3/SFTP (restic), billed at its own price.")

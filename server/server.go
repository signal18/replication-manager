...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.StringVar(&conf.BackupResticReseedStrategy, "backup-restic-reseed-strategy", "auto", "Restic reseed strategy: auto (automatic selection), restore (extract to disk), dump (stream single file), mount (FUSE filesystem)")
	flags.StringVar(&conf.BackupResticReseedTempDir, "backup-restic-reseed-temp-dir", "", "Temporary directory for restic reseed operations (empty = system temp)")

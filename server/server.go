...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	if WithProvisioning == "ON" {
		...
		if WithOpenSVC == "ON" {
			...
			flags.BoolVar(&conf.ProvOpensvcImageForcePull, "opensvc-image-force-pull", false, "Force docker image pull on service start (set automatically during rolling upgrade)")

...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.IntVar(&conf.ProvServicePlanDbu, "prov-service-plan-dbu", 2, "Service plan in Database Units (DBU reservation contract; 1 DBU = 1 core / 4GB / 40GB / 1000 IOPS). Default 2 = a minimal HA cluster (master + 1 replica = 2 nodes).")

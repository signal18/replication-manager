...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.StringVar(&conf.ProvDBConfigPreserveVars, "prov-db-config-preserve-vars", "tmpdir,slave_load_tmpdir,log_error,slow_query_log_file,general_log_file,innodb_data_home_dir,innodb_log_group_home_dir,innodb_undo_directory,log_bin,log_bin_index,relay_log,relay_log_index,slave_load_tmpdir,aria_log_dir_path,server_audit_file_path,sql_error_log_filename", "Comma-separated list of configuration keys to preserve during config change")

...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.BoolVar(&conf.BackupSplitUsers, "backup-split-users", false, "To split backup for each user")
	flags.BoolVar(&conf.BackupRestoreUsers, "backup-restore-users", true, "To restore users from backup")
	flags.BoolVar(&conf.BackupPushFromNode, "backup-push-from-node", false, "To push backup from node to repman server")

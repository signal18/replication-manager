...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	if WithProvisioning == "ON" {
		...
		flags.StringVar(&conf.ProvKubeProxyStorageClass, "prov-kube-proxy-storage-class", "", "Kubernetes only: StorageClass for the proxy PVC (empty uses the cluster's default StorageClass)")

...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.BoolVar(&conf.MCPAuthEnabled, "mcp-auth-enabled", true, "Require Bearer JWT (same as REST API /api/login token) on MCP /sse and /message endpoints; ignored for stdio transport")

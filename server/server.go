...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config, isClient bool) {
	...
	flags.BoolVar(&conf.ProvDBDBUSensor, "prov-db-dbu-sensor", false, "Bind the OpenSVC service pg cgroup read-only into the jobs container (/svc-cgroup) so the dbjob DBU sensor can measure the whole-service consumed DBU (mem/cpu/io); off by default (a bad bind could block container start on an unexpected cgroup layout)")

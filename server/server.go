...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.IntVar(&conf.BackupDiskTresholdCrit, "backup-disk-treshold-crit", 98, "Critical threshold for used space in percentage. If disk usage is above this value, backup will be skipped")
	flags.BoolVar(&conf.BackupEstimateSize, "backup-estimate-size", true, "To estimate size of backup before processing backup")
	flags.IntVar(&conf.BackupGrowthPercentage, "backup-growth-percentage", 150, "Percentage of growth according to last backup to check required space. 100 will be 1:1, means no growth from last backup. Default 150 (50 percent growth)")

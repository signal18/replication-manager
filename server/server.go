...
func (repman *ReplicationManager) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	if WithProvisioning == "ON" {
		...
		flags.StringVar(&conf.ProvAppAgents, "prov-app-agents", "", "Comma seperated list of agents for micro services provisionning")
		flags.StringVar(&conf.ProvAppDiskSize, "prov-app-disk-size", "20", "Disk in g for micro service VM")
		flags.StringVar(&conf.ProvAppCpuCores, "prov-app-cpu-cores", "1", "Number of cpu cores for the micro service VM")
		flags.StringVar(&conf.ProvAppMemory, "prov-app-memory", "256", "Memory in M for micro service VM")
		flags.StringVar(&conf.ProvAppVolumeData, "prov-app-volume-data", "default", "Volume name of the data files")
		flags.StringVar(&conf.ProvAppDockerRunArgs, "prov-app-docker-run-args", "--ulimit nofile=262144:262144 --sysctl net.ipv4.tcp_tw_reuse=1 --sysctl net.core.somaxconn=1024  --sysctl net.ipv4.tcp_fin_timeout=10", "Additional docker run arguments for app")
		flags.StringVar(&conf.ProvAppDockerImg, "prov-app-docker-img", "signal18/repman-app:latest", "Docker image for app")

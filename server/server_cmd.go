...
func initLogFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&conf.LogHistoryEnable, "log-history-enable", true, "Enable the on-disk log history API (browsing rotated log files beyond the in-memory ring buffer)")
	cmd.Flags().IntVar(&conf.LogHistoryMaxScanBytes, "log-history-max-scan-bytes", 52428800, "Max bytes read across all log files per history request (bounded scan, never 0/unbounded)")
	cmd.Flags().IntVar(&conf.LogHistoryMaxLines, "log-history-max-lines", 5000, "Max log lines returned per history request, regardless of the client-requested limit")
	cmd.Flags().IntVar(&conf.LogHistoryMaxFiles, "log-history-max-files", 20, "Max rotated log files opened per history request")

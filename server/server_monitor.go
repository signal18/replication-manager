...
func init() {
	...
	monitorCmd.Flags().BoolVar(&conf.MonitorCheckGrants, "monitoring-check-grants", true, "Check grants for replication and monitoring users, it use DNS Lookup")
	...
	monitorCmd.Flags().IntVar(&conf.LogSSTLevel, "log-sst-level", 0, "Log SST Level")
	...
	monitorCmd.Flags().IntVar(&conf.LogConfigLoadLevel, "log-config-load-level", 0, "Log Config Load Level")
	monitorCmd.Flags().BoolVar(&conf.LogHeartbeat, "log-heartbeat", false, "Log Heartbeat")
	monitorCmd.Flags().IntVar(&conf.LogHeartbeatLevel, "log-heartbeat-level", 0, "Log Hearbeat Level")
	monitorCmd.Flags().BoolVar(&conf.LogFailedElection, "log-failed-election", false, "Log failed election")
	monitorCmd.Flags().IntVar(&conf.LogFailedElectionLevel, "log-failed-election-level", 0, "Log failed election Level")
	...
	monitorCmd.Flags().StringVar(&conf.MonitoringAlertTrigger, "monitoring-alert-trigger", "ERR00027,ERR00042,ERR00087", "List of errno triggering an alert to be send")
	...
	monitorCmd.Flags().StringVar(&conf.OAuthClientID, "api-oauth-client-id", "", "API OAuth Client ID")
	monitorCmd.Flags().StringVar(&conf.OAuthClientSecret, "api-oauth-client-secret", "", "API OAuth Client Secret")
	...
	monitorCmd.Flags().IntVar(&conf.LogVaultLevel, "log-vault-level", 0, "Log level for vault")
	...
	monitorCmd.Flags().IntVar(&conf.LogGitLevel, "log-git-level", 0, "Log GIT Level")
	...
	if WithMySQLRouter == "ON" {
		...
		monitorCmd.Flags().IntVar(&conf.MysqlRouterLogLevel, "mysqlrouter-log-level", 0, "MySQLRouter log debug level")
	...
	monitorCmd.Flags().StringVar(&conf.SchedulerSenderPorts, "scheduler-db-servers-sender-ports", "", "Scheduler TCP port to receive data from db node, consume one port per transfert if not set, pick one available port")

...
func init() {
	...
	monitorCmd.Flags().BoolVar(&conf.LogSST, "log-sst", true, "Log open and close SST transfert")
	...
	monitorCmd.Flags().BoolVar(&conf.LogVault, "log-vault", true, "Log vault debug")
	...
	monitorCmd.Flags().BoolVar(&conf.LogGit, "log-git", true, "To log clone/push/pull from git")
	...
	if WithMySQLRouter == "ON" {
		...
		monitorCmd.Flags().BoolVar(&conf.MysqlRouterDebug, "mysqlrouter-debug", true, "MySQLRouter log debug")

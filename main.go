...
func init() {
	...
	monitorCmd.Flags().BoolVar(&conf.MonitorQueries, "monitoring-query-rules", true, "Monitor query routing from proxies")
	...
	monitorCmd.Flags().StringVar(&conf.ProvTags, "prov-db-tags", "semisync,innodb,noquerycache,threadpool,slow,pfs,compressbinlog,docker,linux,readonly,diskmonitor,sqlerror,compressbinlog", "playbook configuration tags")
	...
	if WithProvisioning == "ON" {
		...
		if WithOpenSVC == "ON" {
			...
			monitorCmd.Flags().StringVar(&conf.ProvOrchestrator, "prov-orchestator", "opensvc", "opensvc|kube|slapos|local")

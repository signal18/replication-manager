...
- billing payloads are **backend-resolved commercial snapshots**, not the source of truth.
...
Also prepare the authoritative local state needed for future CRM settlement payloads.

## Pricing-source alignment

This plan aligns with the Cloud18 pricing model documents as follows:

- `doc/implementation/config/CLOUD18_CREDIT_MODEL.md` defines the unit model,
  pricing modes, and the base DB/App price computation inputs,
- `doc/implementation/config/CLOUD18_APP_HA_DISCOUNT_PLAN.md` defines future app
  structural pricing adjustments that affect billed app price but not the
  technical unit count,
- this sponsorship/settlement plan defines how replication-manager sends the
  **resolved commercial EUR snapshot** to CRM.

The intended chain is:

1. replication-manager measures current usage,
2. replication-manager computes technical units from the credit/unit model,
3. replication-manager applies any applicable structural pricing rules,
4. replication-manager resolves final commercial EUR amounts,
5. CRM settles those resolved amounts without recomputing price from units.

Under this model:

- `db_units` and `app_units` are operational pricing metadata,
- `db_amount` and `app_amount` are the authoritative commercial amounts sent to CRM,
- CRM validates freshness, ordering, and sponsorship binding, but it must not
  recompute price from units, HA metadata, or current pricing tables.
...
- non-secret,
- stored by repman,
- used in CRM settlement payloads.
...
- enough to build settlement payloads without replaying history,
...
- used as the current sponsorship event watermark in settlement payloads.
...
- canonical stable cluster identity used in settlement payloads,
...
## Settlement payload design boundary

Future CRM settlement payloads must be built **only** from the **current
authoritative local state** plus the current local usage/pricing snapshot.

That means they must be built from:

- authoritative local sponsorship state,
- authoritative local settlement state,
- current local usage/pricing snapshot.

For pricing-source clarity:

- in `csv-service-plan`, replication-manager uses the configured plan-price
  snapshot,
- in `global-unit-pricing`, replication-manager computes units from the
  credit/unit model, applies any applicable structural pricing rules, and
  resolves final EUR amounts locally before sending the payload.
...
App HA structural pricing, when enabled in the future, changes only the backend
resolved billed app price that feeds settlement `app_amount`; it does not change
the authoritative technical unit count sent as pricing metadata.

### Minimal CRM payload target

The consumer should eventually send only the minimal current snapshot CRM requested,
...
### Explicit exclusions from CRM payloads
...
10. continue with derived enforcement / reconciliation work (ACL, user, credential
    application),
11. continue with optional side effects (mail/scripts, later CRM send, later git tracking).
...
- `cluster/cluster_get.go`
  - restore authoritative sponsorship state from `sponsorship-state.json` before
    any runtime logic that can emit settlement payloads or process sponsorship
    transitions
...
- authoritative sponsorship state is restored from `sponsorship-state.json` before
  runtime sponsorship transitions or settlement payload generation.
...
- future CRM settlement payloads derived only from local authoritative state.
...
- CRM receives only the minimal current settlement context it actually needs,

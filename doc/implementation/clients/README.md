...
- Backups are encrypted with OpenSSL salted passphrase mode (`openssl enc -aes-256-cbc -salt -pass pass:<passphrase>`). The CLI uses passphrase decryption by default and keeps compatibility fallback for older base64 (`-a`) artifacts.

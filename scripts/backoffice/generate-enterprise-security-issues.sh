...
#
# Generates enterprise-security-issues.json and pushes it to the GitLab pull
...
#     plugins/data/enterprise-security-issues.json to each eligible pull repo
#     via the GitLab Files API.
#   - repman's existing git-pull sync picks up the file and writes it to
#       {ShareDir}/plugins/data/enterprise-security-issues.json
#   - The built-in EnterpriseSecurityPlugin reads from that path and emits
#     findings matching the server's version range.
#
# Usage:
#   ./generate-enterprise-security-issues.sh [OPTIONS]
#
# Options:
...
#
# Deploy:
#   For each eligible instance, copies enterprise-security-issues.json to
#   ../repos/{domain}/{subdomain}-pull/{cluster}/plugins/data/ and git pushes,
#   following the same pattern as inject_plugins.sh.
#
# Daily guard:
#   Records last successful run in .enterprise-security-last-run.
...
#
# CSV source (cve-entries.csv, same directory):
...
DEFAULT_OUTPUT="${REPO_ROOT}/cluster/logplugin/plugins/plugin-enterprise-security/enterprise-security-issues.json"
CSV_FILE="${SCRIPT_DIR}/cve-entries.csv"
...
# Use --force to bypass.
STAMP_FILE="${SCRIPT_DIR}/.enterprise-security-last-run"
...
if ! $FORCE && [[ -f "${STAMP_FILE}" ]]; then
  ...
  if [[ ${ELAPSED} -lt 86400 ]]; then
    ...
    echo "[enterprise-security-issues] Last run was ${HOURS_AGO}h ago — skipping (use --force to override)" >&2
    exit 0
  fi
fi
...
log() { echo "[enterprise-security-issues] $*" >&2; }
...
if [[ -f "${CSV_FILE}" ]]; then
  ...
  log "No cve-entries.csv found — skipping CSV entries"
fi
...
nvd_fetch_cves "mariadb"       "MariaDB" "mariadb"
sleep 6
nvd_fetch_cves "mysql+oracle"  "MySQL"   "oracle"
...
GH_ISSUES_JSON="[]"
if GH_RAW=$(gh_curl "https://api.github.com/repos/signal18/replication-manager/issues?labels=security&state=open&per_page=100" 2>/dev/null); then
...
#    once at the pull repo root: plugins/data/enterprise-security-issues.json
...
while IFS=$'\t' read -r domain subdomain plan; do
  ...
  if $DRY_RUN; then
    log "  [dry-run] would copy to $pull_repo/plugins/data/enterprise-security-issues.json (plan: $plan)"
    continue
  fi
  ...
  cp "${OUTPUT}" "$pull_repo/plugins/data/enterprise-security-issues.json"

  log "  Committing and pushing to $domain/$subdomain-pull (plan: $plan)..."

  git -C "$pull_repo" add "plugins/data/enterprise-security-issues.json"
  git -C "$pull_repo" commit -m "backoffice: update enterprise-security-issues.json (${GENERATED_AT})" 2>/dev/null || {

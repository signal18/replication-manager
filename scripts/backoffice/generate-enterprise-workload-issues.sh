...
#
# Generates enterprise-workload-issues.json and pushes it to the GitLab pull
...
#     plugins/data/enterprise-workload-issues.json to each eligible pull repo
#     via the GitLab Files API.
#   - repman's existing git-pull sync picks up the file and writes it to
#       {ShareDir}/plugins/data/enterprise-workload-issues.json
#   - The built-in EnterpriseWorkloadPlugin reads from that path and emits
#     findings matching the server's version range.
#
# Usage:
#   ./generate-enterprise-workload-issues.sh [OPTIONS]
#
# Options:
...
#
# Deploy:
#   For each eligible instance, copies enterprise-workload-issues.json to
#   ../repos/{domain}/{subdomain}-pull/{cluster}/plugins/data/ and git pushes,
#   following the same pattern as inject_plugins.sh.
#
# Daily guard:
#   Records last successful run in .enterprise-workload-last-run.
...
#
# CSV source (workload-entries.csv, same directory):
...
DEFAULT_OUTPUT="${REPO_ROOT}/cluster/logplugin/plugins/plugin-enterprise-workload/enterprise-workload-issues.json"
CSV_FILE="${SCRIPT_DIR}/workload-entries.csv"
...
# Use --force to bypass.
STAMP_FILE="${SCRIPT_DIR}/.enterprise-workload-last-run"
...
if ! $FORCE && [[ -f "${STAMP_FILE}" ]]; then
  ...
  if [[ ${ELAPSED} -lt 86400 ]]; then
    ...
    echo "[enterprise-workload-issues] Last run was ${HOURS_AGO}h ago — skipping (use --force to override)" >&2
    exit 0
  fi
fi
...
log() { echo "[enterprise-workload-issues] $*" >&2; }
...
if [[ -f "${CSV_FILE}" ]]; then
  ...
  log "No workload-entries.csv found — skipping CSV entries"
fi
...
# Fetch CRITICAL and HIGH severity CVEs — these are the crash, DoS, and
# performance-impacting bugs not covered by the security or replication plugins.
nvd_fetch_cves "mariadb&cvssV3Severity=CRITICAL"       "MariaDB" "mariadb"
sleep 6
nvd_fetch_cves "mysql+oracle&cvssV3Severity=CRITICAL"  "MySQL"   "oracle"
sleep 6
nvd_fetch_cves "mariadb&cvssV3Severity=HIGH"            "MariaDB" "mariadb"
sleep 6
nvd_fetch_cves "mysql+oracle&cvssV3Severity=HIGH"       "MySQL"   "oracle"

# Merge all NVD batches, deduplicate by id, exclude CVEs already in CSV,
# and exclude CVEs already covered by the security or replication advisory files.
CSV_CVE_IDS=$(echo "${CSV_ISSUES_JSON}" | jq -r '.[].cve | select(. != "")' | sort -u)

# Collect CVE IDs from the other two advisory files to avoid overlap.
OTHER_CVE_IDS=""
SEC_JSON="${REPO_ROOT}/cluster/logplugin/plugins/plugin-enterprise-security/enterprise-security-issues.json"
RPL_JSON="${REPO_ROOT}/cluster/logplugin/plugins/plugin-enterprise-replication/enterprise-replication-issues.json"
for f in "$SEC_JSON" "$RPL_JSON"; do
  if [[ -f "$f" ]]; then
    OTHER_CVE_IDS="${OTHER_CVE_IDS}
$(jq -r '.issues[].cve | select(. != "")' "$f")"
  fi
done
ALL_EXCLUDE=$(printf '%s\n%s' "${CSV_CVE_IDS}" "${OTHER_CVE_IDS}" | sort -u)

NVD_ISSUES_JSON=$(cat /tmp/nvd_batch.json | jq -s 'add // []' | jq --arg exclude_cves "${ALL_EXCLUDE}" '
...
GH_ISSUES_JSON="[]"
if GH_RAW=$(gh_curl "https://api.github.com/repos/signal18/replication-manager/issues?labels=workload&state=open&per_page=100" 2>/dev/null); then
...
#    once at the pull repo root: plugins/data/enterprise-workload-issues.json
...
while IFS=$'\t' read -r domain subdomain plan; do
  ...
  if $DRY_RUN; then
    log "  [dry-run] would copy to $pull_repo/plugins/data/enterprise-workload-issues.json (plan: $plan)"
    continue
  fi
  ...
  cp "${OUTPUT}" "$pull_repo/plugins/data/enterprise-workload-issues.json"

  log "  Committing and pushing to $domain/$subdomain-pull (plan: $plan)..."

  git -C "$pull_repo" add "plugins/data/enterprise-workload-issues.json"
  git -C "$pull_repo" commit -m "backoffice: update enterprise-workload-issues.json (${GENERATED_AT})" 2>/dev/null || {

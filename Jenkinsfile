node () {
    ...
    stage ('Build OSC') {
        docker.withRegistry("https://index.docker.io/v1/", "docker-hub") {
	        def newApp = docker.build("signal18/replication-manager:2.2","-f docker/Dockerfile .")
            newApp.push()
            newApp.push 'latest'
	    }
	}
    stage ('Build PRO') {
        docker.withRegistry("https://index.docker.io/v1/", "docker-hub") {
	        def newApp = docker.build("signal18/replication-manager:2.2-pro","-f docker/Dockerfile.pro .")
            newApp.push()
	    }
	}
}
...
type Config struct {
	...
	ProvDBCapBurstDbu                         int                          `mapstructure:"prov-db-cap-burst-dbu" toml:"prov-db-cap-burst-dbu" json:"provDbCapBurstDbu"` // TECHNICAL cgroup headroom: burst DBU ADDED on top of the tier for the container memory cap (so mariadbd is not OOM-killed above prov-db-memory). NOT overcommit (over-consumption) and NOT the plan; the plan is never modified

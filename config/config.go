...
type Config struct {
	...
	ProvDBCapSafetyPct                        int                          `mapstructure:"prov-db-cap-safety-pct" toml:"prov-db-cap-safety-pct" json:"provDbCapSafetyPct"` // shared safety margin for the two consumed-vs-reference checkStates: CheckResourceConsumedOverConfig (vs config -> ResourceConsumedOverConfigAxes, raise resources) and CheckResourceConsumedOverPlan (vs plan/cap -> IsNeedResourceCapUp, cap up). Fires when a node consumes >= (1 - pct/100) of the reference on any axis. Default 15 (85%)

...
type Config struct {
	...
	ProvDBCapSafetyPct                        int                          `mapstructure:"prov-db-cap-safety-pct" toml:"prov-db-cap-safety-pct" json:"provDbCapSafetyPct"` // SATURATION margin: IsNeedResourceCapUp fires when ANY node consumes >= (1 - pct/100) of its per-node config on ANY axis. On-prem/dynamic-config-independent (consumed vs allocated config). Default 15 (fire at 85%)

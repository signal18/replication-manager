...
fpm ${cflags[@]} -C "$builddir"/package -s dir -t deb -n replication-manager-client --description "$description - client package" -p "$builddir/release" --config-files "$builddir"/package/etc/replication-manager/config.toml
...
do
    ...
    fpm ${cflags[@]} -C "$builddir"/package -s dir -t deb -n replication-manager-$flavor --description "$description - $extra_desc" -p "$builddir/release" --config-files "$builddir"/package/etc/replication-manager/config.toml

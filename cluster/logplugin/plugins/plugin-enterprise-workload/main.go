// plugin-enterprise-workload surfaces CRITICAL/HIGH severity bugs impacting
// workload stability — crashes, deadlocks, optimizer regressions, memory leaks.
// See the built-in static version in cluster/logplugin/plugin_enterprise_workload.go.
package main
...
//go:embed enterprise-workload-issues.json
...
func main() {
	...
	if req.PluginDataDir != "" {
		dataFile := filepath.Join(req.PluginDataDir, "enterprise-workload-issues.json")
	...
	if err := json.Unmarshal(raw, &data); err != nil {
		fmt.Fprintf(os.Stderr, "bad enterprise-workload-issues.json: %v\n", err)
		os.Exit(1)
	}
	...
	for _, iss := range data.Issues {
		...
		desc := strings.ReplaceAll(iss.Description, "{server_url}", req.ServerURL)
		desc = strings.ReplaceAll(desc, "{flavor}", sv.Flavor)
		desc = strings.ReplaceAll(desc, "{version}", serverVerStr)

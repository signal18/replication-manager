...
func (proxy *ConsulProxy) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.IntVar(&conf.RegistryConsulLogLevel, "log-level-registry-consul", 1, "Register write and read SRV DNS to consul")
...
func (proxy *ConsulProxy) Init() {
	...
	if cluster.GetMaster() != nil {
		...
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Register consul master ID %s with host %s", "write_"+cluster.GetName(), cluster.GetMaster().URL)
		delservice, err := reg.GetService("write_" + cluster.GetName())
		if err != nil {
			for _, service := range delservice {

				if err := reg.Deregister(service); err != nil {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Unexpected deregister error: %v", err)
				}
			}
		}
		//reg := registry.NewRegistry()
		for _, v := range writesrv {
			for _, service := range v {
				...
				if err := reg.Register(service); err != nil {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Unexpected register error: %v", err)
				}

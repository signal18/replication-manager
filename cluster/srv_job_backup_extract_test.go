...
func TestExtractArchiveToDirRejectsRegularWriteToSymlinkDestination(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink test not supported on windows")
	}
	...
	archivePath := filepath.Join(t.TempDir(), "symlink_dest_overwrite.tar")
	writeTestTar(t, archivePath, []testTarEntry{
		{Name: "root/", Type: tar.TypeDir},
		{Name: "root/safe/", Type: tar.TypeDir},
		{Name: "root/safe/file.txt", Type: tar.TypeReg, Content: "safe"},
		{Name: "root/target", Type: tar.TypeSymlink, Linkname: "safe/file.txt"},
		{Name: "root/target", Type: tar.TypeReg, Content: "compromised"},
	})
	...
	_, err := extractArchiveToDir(archivePath, extractDir)
	if err == nil {
		t.Fatal("expected error for regular write to symlink destination, got nil")
	}
	if !strings.Contains(err.Error(), "destination path is a symlink") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestExtractArchiveToDirRejectsRegularWriteUnderSymlinkParent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink test not supported on windows")
	}

	archivePath := filepath.Join(t.TempDir(), "regular_under_symlink_parent.tar")
	writeTestTar(t, archivePath, []testTarEntry{
		{Name: "root/", Type: tar.TypeDir},
		{Name: "root/linkdir/payload.txt", Type: tar.TypeReg, Content: "payload"},
	})

	rootTmp := t.TempDir()
	extractDir := filepath.Join(rootTmp, "extract")
	outsideDir := filepath.Join(rootTmp, "outside")
	if err := os.MkdirAll(filepath.Join(extractDir, "root"), 0o755); err != nil {
		t.Fatalf("failed to create extract root: %v", err)
	}
	if err := os.MkdirAll(outsideDir, 0o755); err != nil {
		t.Fatalf("failed to create outside dir: %v", err)
	}
	if err := os.Symlink(outsideDir, filepath.Join(extractDir, "root", "linkdir")); err != nil {
		t.Fatalf("failed to create symlinked parent: %v", err)
	}

	_, err := extractArchiveToDir(archivePath, extractDir)
	if err == nil {
		t.Fatal("expected error for regular write under symlinked parent, got nil")
	}
	if !strings.Contains(err.Error(), "parent path contains symlink") {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestExtractArchiveToDirRejectsDirCreateUnderSymlinkParent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink test not supported on windows")
	}

	archivePath := filepath.Join(t.TempDir(), "dir_under_symlink_parent.tar")
	writeTestTar(t, archivePath, []testTarEntry{
		{Name: "root/", Type: tar.TypeDir},
		{Name: "root/linkdir/subdir/", Type: tar.TypeDir},
	})
	...
	_, err := extractArchiveToDir(archivePath, extractDir)
	if err == nil {
		t.Fatal("expected error for dir create under symlinked parent, got nil")
	}
	if !strings.Contains(err.Error(), "parent path contains symlink") {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExtractArchiveToDirRejectsHardlinkDestUnderSymlinkParent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("hardlink/symlink test not supported on windows")
	}

	archivePath := filepath.Join(t.TempDir(), "hardlink_dest_under_symlink_parent.tar")
	writeTestTar(t, archivePath, []testTarEntry{
		{Name: "root/", Type: tar.TypeDir},
		{Name: "root/original.sql", Type: tar.TypeReg, Content: "SELECT 1;"},
		{Name: "root/linkdir/alias.sql", Type: tar.TypeLink, Linkname: "root/original.sql"},
	})
	...
	_, err := extractArchiveToDir(archivePath, extractDir)
	if err == nil {
		t.Fatal("expected error for hardlink dest under symlinked parent, got nil")
	}
	if !strings.Contains(err.Error(), "parent path contains symlink") {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExtractArchiveToDirRejectsSymlinkCreateUnderSymlinkParent(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink test not supported on windows")
	}

	archivePath := filepath.Join(t.TempDir(), "symlink_under_symlink_parent.tar")
	writeTestTar(t, archivePath, []testTarEntry{
		{Name: "root/", Type: tar.TypeDir},
		{Name: "root/safe/", Type: tar.TypeDir},
		{Name: "root/linkdir/newlink", Type: tar.TypeSymlink, Linkname: "../safe"},
	})
	...
	_, err := extractArchiveToDir(archivePath, extractDir)
	if err == nil {
		t.Fatal("expected error for symlink create under symlinked parent, got nil")
	}
	if !strings.Contains(err.Error(), "parent path contains symlink") {
		t.Fatalf("unexpected error: %v", err)
	}

...
func (proxy *SphinxProxy) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.BoolVar(&conf.SphinxDebug, "sphinx-debug", true, "Turn on SphinxSearch detection Debug")
	flags.IntVar(&conf.SphinxLogLevel, "log-level-sphinx", 1, "Set Log Level for SphinxSearch")

...
func (configurator *Configurator) GetFileForWrite(path string, force bool) (*os.File, error) {
	...
	// If first line has no Signature create .new file
	if !strings.HasPrefix(firstLine, "# Generated by Signal18") {
		return os.Create(path + ".new")
	}

	// If first line has Signature replace file
	return os.Create(path)
}

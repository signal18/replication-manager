...
func (server *ServerMonitor) CheckMaxConnections() {
	...
	if curCx > maxCx*80/100 {
		cluster.SetState("ERR00076", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["ERR00076"], ErrFrom: "MON", ServerUrl: server.URL})
	}
}
...
func (server *ServerMonitor) CheckVersion() {
	...
	if server.DBVersion.IsMariaDB() && server.DBVersion.LowerReleaseList("10.4.12", "10.5.1") {
		cluster.SetState("WARN0099", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0099"], ErrFrom: "MON", ServerUrl: server.URL})
	}
...
func (server *ServerMonitor) CheckDisks() {
	...
	for _, d := range server.Disks {
		if d.Used/d.Total*100 > int32(cluster.Conf.MonitorDiskUsagePct) {
			cluster.SetState("ERR00079", state.State{ErrType: "WARNING", ErrDesc: clusterError["ERR00079"], ErrFrom: "MON", ServerUrl: server.URL})
		}
	}
}
...
func (server *ServerMonitor) CheckSlaveSettings() {
	...
	if cluster.Conf.ForceSlaveSemisync && sl.HaveSemiSync == false && cluster.GetTopology() != topoMultiMasterWsrep {
		...
		cluster.SetState("WARN0048", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0048"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	...
	if cluster.Conf.ForceBinlogRow && sl.HaveBinlogRow == false {
		...
		cluster.SetState("WARN0049", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0049"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	...
	if cluster.Conf.ForceSlaveHeartbeat && sl.GetReplicationHearbeatPeriod() > 1 {
		...
		cluster.SetState("WARN0050", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0050"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if cluster.Conf.ForceSlaveGtid && sl.GetReplicationUsingGtid() == "No" {
		...
		cluster.SetState("WARN0051", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0051"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if cluster.Conf.ForceSlaveGtidStrict && !sl.IsReplicationUsingGtidStrict() && cluster.GetTopology() != topoMultiMasterWsrep && server.IsMariaDB() {
		...
		cluster.SetState("WARN0058", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0058"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	...
	if cluster.Conf.ForceSlaveIdempotent && !sl.HaveSlaveIdempotent && cluster.GetTopology() != topoMultiMasterWsrep && server.IsMariaDB() {
		...
		cluster.SetState("WARN0103", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0103"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}*/
	if cluster.Conf.ForceSlaveStrict && sl.HaveSlaveIdempotent && cluster.GetTopology() != topoMultiMasterWsrep && server.IsMariaDB() {
		...
		cluster.SetState("WARN0104", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0103"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	} */
	...
	if cluster.Conf.ForceSyncInnoDB && sl.HaveInnodbTrxCommit == false {
		...
		cluster.SetState("WARN0052", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0052"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if cluster.Conf.ForceBinlogChecksum && sl.HaveChecksum == false {
		...
		cluster.SetState("WARN0053", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0053"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if cluster.Conf.ForceBinlogSlowqueries && sl.HaveBinlogSlowqueries == false {
		...
		cluster.SetState("WARN0054", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0054"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if cluster.Conf.ForceBinlogAnnotate && sl.HaveBinlogAnnotate == false && server.IsMariaDB() {
		...
		cluster.SetState("WARN0055", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0055"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	...
	if cluster.Conf.ForceBinlogCompress && sl.HaveBinlogCompress == false && sl.DBVersion.IsMariaDB() && sl.DBVersion.Major >= 10 && sl.DBVersion.Minor >= 2 {
		...
		cluster.SetState("WARN0056", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0056"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
	if sl.IsIgnored() == false && sl.HaveBinlogSlaveUpdates == false {
		cluster.SetState("WARN0057", state.State{ErrType: config.LvlWarn, ErrDesc: clusterError["WARN0057"], ErrFrom: "TOPO", ServerUrl: sl.URL})
	}
...
func (server *ServerMonitor) CheckMasterSettings() {
	...
	if cluster.Conf.ForceSlaveSemisync && server.HaveSemiSync == false {
		...
		cluster.SetState("WARN0060", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0060"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceBinlogRow && server.HaveBinlogRow == false {
		...
		cluster.SetState("WARN0061", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0061"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceSyncBinlog && server.HaveBinlogSync == false {
		...
		cluster.SetState("WARN0062", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0062"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceSyncInnoDB && server.HaveBinlogSync == false {
		...
		cluster.SetState("WARN0064", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0064"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceBinlogAnnotate && server.HaveBinlogAnnotate == false && server.IsMariaDB() {
		...
		cluster.SetState("WARN0067", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0067"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceBinlogChecksum && server.HaveChecksum == false {
		...
		cluster.SetState("WARN0065", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0065"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if cluster.Conf.ForceBinlogCompress && server.HaveBinlogCompress == false && server.IsMariaDB() && server.DBVersion.Major >= 10 && server.DBVersion.Minor >= 2 {
		...
		cluster.SetState("WARN0068", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0068"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if server.HaveBinlogSlaveUpdates == false {
		cluster.SetState("WARN0069", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0069"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if server.HaveGtidStrictMode == false && server.DBVersion.Flavor == "MariaDB" && cluster.GetTopology() != topoMultiMasterWsrep && cluster.GetTopology() != topoMultiMasterGrouprep {
		cluster.SetState("WARN0070", state.State{ErrType: "WARNING", ErrDesc: clusterError["WARN0070"], ErrFrom: "TOPO", ServerUrl: server.URL})
	}
	if server.IsAcid() == false && cluster.IsDiscovered() {
	...
	if server.State != "" && !server.IsDown() && server.IsRelay == false {
		...
		if err != nil {
			...
			if priv.Repl_client_priv == "N" {
				cluster.SetState("ERR00006", state.State{ErrType: "ERROR", ErrDesc: clusterError["ERR00006"], ErrFrom: "CONF", ServerUrl: server.URL})
			}
			if priv.Super_priv == "N" {
				cluster.SetState("ERR00008", state.State{ErrType: "ERROR", ErrDesc: clusterError["ERR00008"], ErrFrom: "CONF", ServerUrl: server.URL})
			}
			if priv.Reload_priv == "N" {
				cluster.SetState("ERR00009", state.State{ErrType: "ERROR", ErrDesc: clusterError["ERR00009"], ErrFrom: "CONF", ServerUrl: server.URL})
			}
		}
		...
		for _, sv2 := range cluster.Servers {
			if sv2.URL != server.URL && sv2.IsRelay == false && !sv2.IsDown() {
				...
				if rpriv.Repl_slave_priv == "N" {
					cluster.SetState("ERR00007", state.State{ErrType: "ERROR", ErrDesc: clusterError["ERR00007"], ErrFrom: "CONF", ServerUrl: sv2.URL})
				}
			}
		}
	}

...
func (proxy *MariadbShardProxy) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.BoolVar(&conf.MdbsProxyDebug, "shardproxy-debug", true, "MariaDB Spider proxy")
	flags.IntVar(&conf.MdbsProxyLogLevel, "shardproxy-log-level", 1, "MariaDB Spider proxy")
...
func (proxy *MariadbShardProxy) Init() {
	...
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Init MdbShardProxy %s %s", proxy.Host, proxy.Port)
...
func (proxy *MariadbShardProxy) Failover() {
	...
	if err != nil {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Could not fetch master schemas %s", err)
	}
...
func (cluster *Cluster) CheckMdbShardServersSchema(proxy *MariadbShardProxy) {
	...
	for _, s := range schemas {
		...
		_, err = proxy.ShardProxy.Conn.Exec(query)
		if err != nil {
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "query %s %s", query, err)
		}
		for _, slave := range cluster.slaves {
		...
		if err != nil {
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Failed query %s %s", query2, err)
		}
...
func (cluster *Cluster) ShardProxyCreateVTable(proxy *MariadbShardProxy, schema string, table string, duplicates []*ServerMonitor, withreshard bool) error {
	...
	if len(duplicates) == 1 {
		...
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Creating split table in MdbShardProxy %s", schema+"."+table)
		...
		if err != nil {
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Failed Founding hash key %s %s", query, err)
			return err
		}
		hashFunc := "HASH"
		...
		ddl, err = cluster.GetTableDLLNoFK(schema, table, cluster.master)
		if err != nil {
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlWarn, "Failed query %s %s", query, err)
...
func (cluster *Cluster) ShardProxyReshardTable(proxy *MariadbShardProxy, schema string, table string, clusters map[string]*Cluster) error {
	...
	for _, pri := range cluster.Proxies {
		if pr, ok := pri.(*MariadbShardProxy); ok {
			...
			ct := 0
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Online data copy...")
			...
			for {
				...
				//	err = cluster.RunQueryWithLog(pr.ShardProxy, query)
				if err != nil {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "copy error...", err)
					if ct == 2 {
						return err
					}
				} else {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Copy Result... %d", res)
					break
				}
				ct++
			}
...
func (cluster *Cluster) RunQueryWithLog(server *ServerMonitor, query string) error {
	...
	if err != nil {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Sharding Proxy %s %s %s", server.URL, query, err)
		return err
	}
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlInfo, "Sharding Proxy %s %s", server.URL, query)
	return nil
}
...
func (cluster *Cluster) ShardProxyCreateSystemTable(proxy *MariadbShardProxy) error {
	...
	if err != nil {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Could initialize MariaDB Sharding Proxy %s", err)
		return err
	}
	...
	if c != nil {
		...
		_, err = c.Exec(sql)
		if err != nil {
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModProxy, config.LvlErr, "Failed query %s", err)
			return err
		}

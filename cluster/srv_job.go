...
// checksumScriptIgnoreGenline computes a SHA-256 checksum of a script file
// while stripping the GENLINE comment (which contains a timestamp that changes
// on every config regeneration). Without this, the checksum is always different
// and WARN0147 fires permanently.
func checksumScriptIgnoreGenline(path string) (string, error) {
	...
	for _, line := range lines {
		if strings.Contains(line, "Generated by Signal18 replication-manager") {
			continue
		}
...
func (server *ServerMonitor) CheckJobsVersion() error {
	...
	if err != nil {
		...
		sum, checkerr = checksumScriptIgnoreGenline(currentScriptPath)
	}
	...
	newsum, err = checksumScriptIgnoreGenline(scriptPath)
	if err != nil {
		checkerr = err
	}

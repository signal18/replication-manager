			...
			if err == nil {
				if extraUrl != "" {
					for j, url := range strings.Split(extraUrl, ",") {
						...
						srv, err := cluster.newServerMonitor(url, cluster.GetDbUser(), cluster.GetDbPass(), true, cluster.GetDomain(), cluster.Name)
		...
		if cluster.Conf.Heartbeat {
			for _, s2 := range cluster.Servers {
				...
				if url2 != url {
					...
					if err != nil {
						cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModHeartBeat, config.LvlWarn, "Can not set heartbeat table to %s", url)
						return
					}

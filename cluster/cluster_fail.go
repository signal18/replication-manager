...
func (cluster *Cluster) MasterFailover(fail bool) bool {
	...
	if !cluster.Conf.MultiMaster && !cluster.Conf.MultiMasterGrouprep {
		...
		if cluster.master.DBVersion.IsMariaDB() || (!cluster.master.DBVersion.IsMariaDB() && cluster.master.DBVersion.Minor < 7) {
	...
	for _, sl := range cluster.slaves {
		...
		if sl.URL == cluster.oldMaster.URL || sl.State == stateMaster || (!sl.IsRelay && cluster.Conf.MxsBinlogOn) {
			continue
		}
		...
		if !sl.HasMariaDBGTID() && !cluster.master.HasMySQLGTID() {
			...
			if cluster.Conf.AutorejoinSlavePositionalHeartbeat {
				...
				changemasteropt.Mode = "POSITIONAL"
				_, changeMasterErr = dbhelper.ChangeMaster(sl.Conn, changemasteropt, sl.DBVersion)
		...
		if cluster.Conf.ReadOnly && !cluster.Conf.MxsBinlogOn && !cluster.IsInIgnoredReadonly(sl) {
	...
	if cluster.Conf.FailoverSwitchToPrefered && fail && cluster.Conf.PrefMaster != "" && !cluster.master.IsPrefered() {
...
func (cluster *Cluster) electSwitchoverCandidate(l []*ServerMonitor, forcingLog bool) int {
	...
	for i, sl := range l {
		...
		if cluster.IsInPreferedHosts(sl) {
			if (cluster.Conf.HasLogLevelPos(15) || forcingLog) && cluster.IsInFailover() {
				cluster.LogPrintf(LvlDbg, "Election rig: %s elected as preferred master", sl.URL)
			}
			return i
		}
...
func (cluster *Cluster) isSlaveElectable(sl *ServerMonitor, forcingLog bool) bool {
	...
	if sl.IsMaintenance {
		...
		if cluster.Conf.HasLogLevelPos(15) || cluster.Conf.Verbose || forcingLog {
...
func (cluster *Cluster) VMasterFailover(fail bool) bool {
	...
	// Phase 2: Reject updates and sync slaves on switchover
	if !fail && cluster.GetTopology() != topoMultiMasterWsrep {
...
func (cluster *Cluster) CloseRing(oldMaster *ServerMonitor) error {
	...
	// Not MariaDB and not using MySQL GTID, 2.0 stop doing any thing until pseudo GTID
	if parent.DBVersion.IsMySQLOrPerconaGreater57() && hasMyGTID {

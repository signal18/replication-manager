...
// TestOpenSVCGetHaproxyContainerSectionStandbyRunsCheckConfig guards against
// the OpenSVC HAProxy container always launching with the image's default
// entrypoint/CMD, which reads haproxy.cfg (proxy_cnf_haproxy_runtime_api, no
// external-check) regardless of haproxy-mode. For haproxy-mode=standby, the
// container must instead run haproxy_check.cfg -- the file with
...
// exit immediately, the same reasoning as k8sProxyDeployment's standby
// branch (cluster/prov_k8s_prx.go).
func TestOpenSVCGetHaproxyContainerSectionStandbyRunsCheckConfig(t *testing.T) {
...
// against the standby-only command/entrypoint override leaking into

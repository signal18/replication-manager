...
// default entrypoint/CMD, which reads haproxy.cfg (proxy_cnf_haproxy_
// runtime_api, no external-check) regardless of haproxy-mode. For
// haproxy-mode=externalcheck, the container must instead run
// haproxy_check.cfg -- the file with checkmaster/checkslave wired in via
// "option external-check" -- or nothing ever excludes a broken replica from
// the write group (matching a real production incident: replicas in
// SLAVE_ERROR stayed UP in the write backend indefinitely). "-db" is
// required because haproxy_check.cfg's "daemon" directive would otherwise
// background the process and let PID 1 exit immediately, the same reasoning
// as k8sProxyDeployment's standby branch (cluster/prov_k8s_prx.go).
...
// haproxy-mode=standby counterpart: standby has no external check either --
// it's Init() (cluster/prx_haproxy.go) deciding read-backend membership from
// replication state directly -- so it must not get the externalcheck-only
// override either.

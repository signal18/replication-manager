...
func (cluster *Cluster) WaitFailoverEndState() {
	for cluster.StateMachine.IsInFailover() {
		...
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting for failover stopped.")
	}
...
func (cluster *Cluster) WaitFailover(wg *sync.WaitGroup) {
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting failover end")
	defer wg.Done()
	exitloop := 0
	...
	for int64(exitloop) < cluster.Conf.MonitorWaitRetry {
		...
		case <-cluster.failoverCond.Recv:
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Failover end receive from channel failoverCond")
			return
		}
	}
	if exitloop == 9999999 {
...
func (cluster *Cluster) WaitSwitchover(wg *sync.WaitGroup) {
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting switchover end")
	defer wg.Done()
	exitloop := 0
...
func (cluster *Cluster) WaitRejoin(wg *sync.WaitGroup) {
	...
	defer wg.Done()
	logline := cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting Rejoin")
	exitloop := 0
	...
	for int64(exitloop) < cluster.Conf.MonitorWaitRetry {
		...
		select {
		case <-ticker.C:
			cluster.LogUpdate(logline, config.LvlInfo, "Waiting Rejoin %d", exitloop)
			exitloop++
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Rejoin Finished")
...
func (cluster *Cluster) WaitClusterStop() error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Cluster is shutdown")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Cluster shutdown timeout")
...
func (cluster *Cluster) WaitProxyEqualMaster() error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Proxy can join master")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Proxy to join master timeout")
...
func (cluster *Cluster) WaitMariaDBStop(server *ServerMonitor) error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "MariaDB shutdown")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "MariaDB shutdown timeout")
...
func (cluster *Cluster) WaitDatabaseSuspect(server *ServerMonitor) error {
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Wait state suspect on %s", server.URL)
	exitloop := 0
	ticker := time.NewTicker(time.Millisecond * time.Duration(cluster.Conf.MonitoringTicker*1000))
	for int64(exitloop) < cluster.Conf.MonitorWaitRetry {
		select {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting state suspect reach on %s", server.URL)
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Wait state suspect timeout on %s", server.URL)
...
func (cluster *Cluster) WaitDatabaseFailed(server *ServerMonitor) error {
	cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Waiting state failed on %s", server.URL)
	exitloop := 0
	ticker := time.NewTicker(time.Millisecond * time.Duration(cluster.Conf.MonitoringTicker*1000))
	for int64(exitloop) < cluster.Conf.MonitorWaitRetry {
		select {
...
func (cluster *Cluster) WaitBootstrapDiscovery() error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Cluster is bootstraped and discovered")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Bootstrap timeout")
...
func (cluster *Cluster) waitMasterDiscovery() error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "Master founded")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Master found timeout")
...
func (cluster *Cluster) WaitDatabaseCanConn() error {
	...
	if exitloop == 9999999 {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlInfo, "All databases can connect")
	} else {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Timeout waiting for database to be connected")

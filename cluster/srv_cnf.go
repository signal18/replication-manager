...
func (server *ServerMonitor) GetEnv() map[string]string {
	...
	return map[string]string{
		...
		"%%ENV:SVC_CONF_ENV_MAX_SESSION_MEM_USED%%":                 strconv.FormatInt(server.ClusterGroup.Configurator.GetConfigMaxSessionMemUsedMB(), 10),
		"%%ENV:SVC_CONF_ENV_TMP_TABLE_SIZE%%":                       server.ClusterGroup.Configurator.GetConfigTmpTableSize(),
		...
		"%%ENV:SVC_CONF_ENV_PFS_DIGEST_LENGTH%%":                    server.ClusterGroup.Configurator.GetConfigPFSDigestLength(),
...
func (server *ServerMonitor) sendDummyConfigWithWait(configFile string, cookieModTime time.Time) error {
	...
	// progress=nil: this is a config-sync transfer, not a reseed -- see
	// SSTProgressSink's doc comment for why it must not write reseed progress
	// counters (could corrupt an unrelated reseed in progress on this same
	// server).
	err := cluster.SSTRunSender(configFile, server, false, nil)
	if err != nil {
		cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr,

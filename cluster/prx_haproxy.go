...
func (proxy *HaproxyProxy) AddFlags(flags *pflag.FlagSet, conf *config.Config) {
	...
	flags.IntVar(&conf.HaproxyLogLevel, "haproxy-log-level", 0, "Log level for debug")
	...
	*/
		...
		if strings.Contains(strings.ToLower(line[0]), "read") {
			...
			if srv != nil {
				...
				if srv.IsMaintenance && line[17] == "UP" {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModHAProxy, LvlInfo, "HaProxy detecting server %s in maintenance but proxy %s report UP  ", srv.URL, proxy.Host+":"+proxy.Port)
					proxy.SetMaintenance(srv)
				}
				if !srv.IsMaintenance && line[17] == "MAINT" {
					cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModHAProxy, LvlInfo, "HaProxy detecting server %s up but proxy %s report in maintenance ", srv.URL, proxy.Host+":"+proxy.Port)
					proxy.SetMaintenance(srv)
				}
			}
		}
	...
	if !foundMasterInStat {
		...
		if master != nil && master.IsLeader() {
			...
			cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModHAProxy, LvlInfo, "Haproxy have leader in cluster but not in haproxy %s fixing it to master %s return %s", proxy.Host+":"+proxy.Port, master.URL, res)
			if err != nil {
				cluster.LogModulePrintf(cluster.Conf.Verbose, config.ConstLogModHAProxy, LvlErr, "Haproxy can not add leader %s in cluster but not in haproxy %s : %s", master.URL, proxy.Host+":"+proxy.Port, err)
			}
		}
	}
	return nil

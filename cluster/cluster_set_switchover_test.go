...
func TestSetSwitchoverWaitTrx(t *testing.T) {
	c := &Cluster{Conf: &config.Config{SwitchWaitTrx: 10}}
	if err := c.SetSwitchoverWaitTrx("3600"); err != nil || c.Conf.SwitchWaitTrx != 3600 {
		t.Fatalf("3600 must be accepted, got err=%v value=%d", err, c.Conf.SwitchWaitTrx)
	}
	for _, bad := range []string{"0", "-5", "abc", ""} {
		if err := c.SetSwitchoverWaitTrx(bad); err == nil {
			t.Fatalf("%q must be rejected", bad)
		}
	}
	if c.Conf.SwitchWaitTrx != 3600 {
		t.Fatalf("a rejected value must not change the setting, got %d", c.Conf.SwitchWaitTrx)
	}
}

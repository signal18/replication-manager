...
**OpenSVC modulesets are generated — NEVER hand-edit**: `share/opensvc/moduleset_*.json` are exports generated by the OpenSVC collector, which is the reference. To change a config template: update the ruleset variable on the collector (UI or `SetRulesetVariableValue`), then export and copy the JSON into `share/`. A code change that needs a template change ships the Go getter + `%%ENV:...%%` placeholder wiring only (inert until the ruleset references it) plus a PR section giving the exact ruleset text — see PR #1753 for the pattern. Instances pick updated modulesets up automatically via the enterprise-compliance plugin (`prov-auto-update-compliance`, preserved variables never overwritten).

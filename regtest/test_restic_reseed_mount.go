...
// TestResticReseedMount validates reseeding a replica from a restic snapshot using the mount strategy.
func (regtest *RegTest) TestResticReseedMount(cl *clusterpkg.Cluster, conf string, test *clusterpkg.Test) bool {
	...
	// Ensure logical backup type uses mydumper (directory-based) to exercise mount strategy.
	...
	if cl.ResticManager != nil && cl.ResticManager.IsMountDisabled() {
		cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, config.LvlWarn, "Restic mount disabled; skipping mount strategy test")
		return true
	}
	...
	cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, "TEST", "Reseeding replica from restic snapshot %s (mount strategy)", snapshotID)
	opts := clusterpkg.ResticReseedOptions{Overwrite: "if-newer"}
	if err := slave.JobReseedFromRestic(snapshotID, "logical", "mount", opts); err != nil {
	...
	cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, "TEST", "Restic reseed mount test succeeded")
	return true
}

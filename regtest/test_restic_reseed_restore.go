...
// TestResticReseedRestore validates reseeding a replica from a restic snapshot using the restore strategy.
func (regtest *RegTest) TestResticReseedRestore(cl *clusterpkg.Cluster, conf string, test *clusterpkg.Test) bool {
	...
	// Wait for restic snapshot ID to be available.
	snapshotID := ""
	...
	// Ensure snapshot metadata is ready.
	metaReady := false
	deadline = time.Now().Add(2 * time.Minute)
	for time.Now().Before(deadline) {
	...
	cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, "TEST", "Reseeding replica from restic snapshot %s", snapshotID)
	opts := clusterpkg.ResticReseedOptions{Overwrite: "if-newer"}
	if err := slave.JobReseedFromRestic(snapshotID, "logical", "restore", opts); err != nil {
		cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, config.LvlErr, "Restic reseed failed: %s", err)
		return false
	}

	// Wait briefly for reseed to settle.
	...
	cl.LogModulePrintf(cl.Conf.Verbose, config.ConstLogModGeneral, "TEST", "Restic reseed restore test succeeded")
	return true
}

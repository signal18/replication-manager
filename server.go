...
func init() {
	...
	if WithBackup == "ON" {
		...
		monitorCmd.Flags().StringVar(&conf.BackupLogicalCron, "backup-logical-cron-entry", "0 1 * * 0", "A cron expression represents a set of times, using 6 space-separated fields.")
		monitorCmd.Flags().StringVar(&conf.BackupPhysicalCron, "backup-physical-cron-entry", "0 0 0 * * *", "A cron expression represents a set of times, using 6 space-separated fields.")
